...
(LOOP2)
	...
	// ***** calculating x = j * 512 + i 
	// ***** store x in R4 

	LOADI R4, 9			// multiply by 512 = 9 bits shift left
	MUL R4, R2, R4		// j * 512
	ADD R4, R4, R3		// j * 512 + i = x
	...
	LDMAR AC
	NOP
	LOAD AC				// AC = image(x-512)

	MUL AC, AC, R5		// AC = image(x-512) * 2
	ADD R6, R6, AC		// sum 
	...
	// ***** store total in memory[x-513]
	
	SUB AC, R4, R7		// x - 513
	LDMAR AC
	NOP
	STORE R6			// store total in memory[x-513]	
...
(LOOP4)
	...
	// ***** calculating y = 1024*j + 2*i
	// ***** store the result in R5

	LOADI R5, 10		// no of bits to shift left i.e. multiply by 1024
	MUL R5, R3, R5		// j * 1024
	LDACI 1				// no of bits to shift left i.e. multiply by 2
	MUL AC, R4, AC		// i * 2
	ADD R5, R5, AC		// y = j * 1024 + i * 2
